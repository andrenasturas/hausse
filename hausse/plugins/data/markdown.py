...
class Markdown(Plugin):
    ...
    def __init__(self, selection = None, extras: List[str] = None):
        ...
        self.extras = extras if extras is not None else ['cuddled-lists', 'fenced-code-blocks', 'footnotes', 'header-ids', 'markdown-in-html', 'metadata', 'noreferrer', 'tag-friendly', 'task_list']
    ...
    def __call__(self, elements: List[Element], metadata: dict, settings: dict):
        ...
        for element in self.selection(elements, metadata, settings):
            m = markdown(element._contents, extras=self.extras)
